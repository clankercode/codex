...
- Start (or resume) a thread: Call `thread/start` to open a fresh conversation. The response returns the thread object and you’ll also get a `thread/started` notification. If you’re continuing an existing conversation, call `thread/resume` with its ID instead. If you want to branch from an existing conversation, call `thread/fork` to create a new thread id with copied history. If you want to create a fresh thread whose entire starting history comes from host-supplied messages, call `thread/import_transcript`. Like `thread/start`, `thread/fork`, and `thread/import_transcript`, ephemeral threads stay in memory only.
...
- Begin a turn: To send user input, call `turn/start` with the target `threadId` and the user's input. Optional fields let you override model, cwd, sandbox policy, approval policy, approvals reviewer, base instructions, developer instructions, etc. This immediately returns the new turn object. The app-server emits `turn/started` when that turn actually begins running.
...
- `thread/import_transcript` — create a fresh thread from host-supplied typed messages (`user`, `assistant`, `developer`) so those messages become the new thread’s initial model-visible history. This is useful when a host wants to apply its own summary/state reconstruction policy instead of reusing the source thread’s native history. Optional `sourceThreadId` inherits defaults such as cwd and persisted model metadata from an existing thread, and the importing connection is unsubscribed from that source thread after success so normal idle cleanup can reclaim it if unused.
...
- `turn/start` — add user input to a thread and begin Codex generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. Optional `baseInstructions` and `developerInstructions` replace the session defaults for this and later turns. For `collaborationMode`, `settings.developer_instructions: null` means "use built-in instructions for the selected mode".
...
- `thread/inject_messages` — append typed text messages (`user`, `assistant`, or `developer`) to a loaded thread’s model-visible history without starting a user turn; returns `{}` on success.
...
### Example: Inject typed history messages

Use `thread/inject_messages` when plain-text messages are enough and you do not want to construct raw Responses API items yourself.

```json
{ "method": "thread/inject_messages", "id": 37, "params": {
    "threadId": "thr_123",
    "messages": [
        { "role": "developer", "text": "Keep responses machine-readable." },
        { "role": "assistant", "text": "Previously computed context." }
    ]
} }
{ "id": 37, "result": {} }
```
